    ...
    (err, user) => {
      ...
      req.login(payload, {session: false}, (error) => {
       ...
       const token = jwt.sign(payload, user.email, { expiresIn: '1h' });
        ...
        res.cookie('isLoggedIn', true, domain='127.0.0.1', {httpOnly: true, secure: false, expires: new Date(Date.now() + 7*24*3600000), SameSite: false});
        res.cookie('email', JSON.stringify(user.email), domain='127.0.0.1', {httpOnly: true, secure: false, expires: new Date(Date.now() + 7*24*3600000), SameSite: false});
